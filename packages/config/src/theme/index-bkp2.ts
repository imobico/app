...
import type { ColorProp } from 'tamagui'
import {
  blue,
  blueDark,
  gray,
...
export const palettes = (() => {
  ...
  const getColorPalette = (colors: ColorProp, accentColors: ColorProp): string[] => {
    ...
    return [
      accentBackground,
      transparent(colorPalette[0] as string, 0),
      transparent(colorPalette[0] as string, 0.25),
      transparent(colorPalette[0] as string, 0.5),
      transparent(colorPalette[0] as string, 0.75),
      ...colorPalette,
      transparent(colorPalette[colorI] as string, 0.75),
      transparent(colorPalette[colorI] as string, 0.5),
      transparent(colorPalette[colorI] as string, 0.25),
      transparent(colorPalette[colorI] as string, 0),
      accentColor,
    ] as string[]
  }
      ...
      (key, index) =>
        [
          ...
          getColorPalette(
            ...
            colorTokens.light[
              lightColorNames[
                (index + 1) % lightColorNames.length
              ] as keyof typeof colorTokens.light
            ],
          ),
        ] as const,
    ),
      ...
      (key, index) =>
        [
          ...
          getColorPalette(
            ...
            colorTokens.light[
              darkColorNames[(index + 1) % darkColorNames.length] as keyof typeof colorTokens.dark
            ],
          ),
        ] as const,
    ),
...
export const getTemplates = (scheme: 'dark' | 'light') => {
  ...
  const inverseSurface1 = {
    ...
    background: base.accentBackground,
...
export const themes = themesIn as any
